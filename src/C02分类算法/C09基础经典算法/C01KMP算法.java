 ...
 * 			定义甲,乙分别为str1和str2比较时的指针,如果乙指向了str2的尾部元素时都相等.则说明找到了此子串,在str1中的起始位置为:甲-乙
...
public class C01KMP算法 {
	...
	private int[] getNextArray(char[] arrayStr2) {
		...
		int cn = next[1];	//代表每次要与i-1处的next值,进行比较的值的索引(当i=2时cn=0,因为i-1处的next值为0)
		//cn即代表位置,又代表前缀的长度
