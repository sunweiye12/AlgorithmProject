 ...
 * 		否则将头部删除,然后再尝试添加(知道没有元素或找到被自己大的元素)
 * 		(***之所以可以直接将小于等于的值删掉是因为,此题球最大值,所以此窗口的后面有比你还大的,而且还比你晚退休,所以这些不可能是最大的值)
 * 	  2.窗口删除元素:窗口的尾部每次删除会检查将要删除的元素的下标是否为,队列尾部元素的key,如果是就从尾部删除此对列,否则不处理	
